...
class CombinedDetector(Node):
    ...
    def timer_callback(self):
        ...
        if not self.locked and moments["m00"] > 10000:
            ...
            if self.is_centered((cx-15, cy-15, 30, 30), frame.shape):
                ...
                self.get_logger().info("Landing pad locked via color detection.")
                self.locked = True
                self.locked_status = status_msg.data
            else:
                ...
                if 1000 < area < 10000:
                    ...
                    if len(approx) >= 4:
                        ...
                        if self.is_centered((x, y, w, h), frame.shape):
                            ...
                            self.get_logger().info("Landing pad locked via edge detection.")
                            self.locked = True
                            self.locked_status = status_msg.data
                        else:
        ...
        if not detected:
            status_msg.data = self.locked_status if self.locked else "SEARCHING_MARKER"
